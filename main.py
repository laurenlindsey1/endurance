    ...
    def get(self):
        ...
        if user:
            greeting = ('Welcome, %s! (<a href="%s">sign out</a>)' %
                (user.nickname(), users.create_logout_url('/profile')))
        else:
...
class WorkoutHistoryHandler(webapp2.RequestHandler):
    def get(self):
...
class WorkoutsHistoryHandle(webapp2.RequestHandler):
    def get(self):
        template=env.get_template('workout_history.html')
        self.response.write(template.render())
        #templates:
...
class ProfileHandler(webapp2.RequestHandler):
   ...
   def post(self):
        template=env.get_template('main.html')
        user = User(name=self.request.get('name'), int(workout_awards=self.request.get('height')), int(weight=self.request.get('weight')))
        user.put()
...
app = webapp2.WSGIApplication([
    ...
    ('/workout_history', WorkoutHistoryHandler)
